...
PDF → [Phase 1] 解析・プロンプト生成
      ...
      [fill] テキスト補完 & STEP 2プロンプト更新（スクリプト自動）
        ↓
        STEP 1.6: 視覚的ボーダー修正（任意・Vision LLM）
        STEP 2: Pythonコード生成
         ↓
...
| Phase 1.5後処理 | `fill` | スクリプト | STEP 1.5 出力JSONに欠落テキストを自動補完し、STEP 2 プロンプトを更新 |
...
### Phase 2（続き）：STEP 1.6 / STEP 2

#### STEP 1.6 — 視覚ボーダー修正（任意・Vision LLM使用時のみ）

Vision LLM（画像入力対応のLLM）を使う場合のオプションステップです。

1. `prompts/<pdf_name>_prompt_step1_6.txt` とページ画像（`images/` 内）をLLMに添付して投入する
2. 出力されたJSONで `_step1_5_input.json` の内容を更新し、再度 `fill` を実行する
...
### STEP 1.6（視覚ボーダー修正・任意）
Vision LLM（画像入力対応）を使ってページ画像と検出ボーダーを目視比較し、罫線の誤検出・未検出を修正します。
