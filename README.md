...
| 処理 | 内容 |
|------|------|
| PDF解析 | `pdfplumber` でテキスト・罫線・矩形を抽出し、コンテンツ境界ベースで Excel グリッド座標を計算 |
...
| 元PDFコピー | 元のPDFファイルを出力先フォルダにコピー（生成Excelとの比較用） |
...
グリッド座標の計算は、ページ全体ではなく **コンテンツが存在する領域（コンテンツ境界）** を基準に行います。

1. 全ワード・矩形・テーブルセルの座標からコンテンツ領域の `min_x, max_x, min_y, max_y` を検出
2. コンテンツ領域の幅・高さを `max_cols` / `max_rows` で分割してセルサイズを算出
3. 各要素のPDF座標をコンテンツ境界からの相対位置でグリッド行・列番号に変換

これにより、余白の多いPDFでもコンテンツ部分にグリッドが集中し、Excelセルを効率的に使えます。プレビュー画像ではコンテンツ範囲外がグレー表示されます。
...
│   ├── core/
│   │   ├── pipeline.py        # パイプラインオーケストレーション（SheetlingPipeline）
│   │   ├── grid.py            # グリッド座標計算・用紙サイズ検出
│   │   └── layout.py          # レイアウトJSON生成（テキスト・罫線要素）
│   ├── parser/
...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| `pdfplumber` | 0.11.9 | PDF内のテキスト、表、罫線の詳細な座標情報を抽出 |
| `openpyxl` | 3.1.5 | Excelファイルの生成、セルのスタイリング、印刷範囲設定 |
| `Pillow` | *(Dockerイメージ提供)* | 罫線プレビュー画像の生成（コンテンツ範囲外のグレーアウト含む） |
| `pytest` | 9.0.3 | テストフレームワーク |

> [!NOTE]
> `requirements.txt` では全パッケージをバージョン完全固定（`==`）で管理しています。
> 意図しないバージョンアップによる動作変化・セキュリティリスクを防ぎ、環境間の再現性を保証するためです。
> バージョンを更新する際は動作確認とテストスイートの通過を確認してください。

---

## 開発メモ

- `src/`、`data/`、`tests/` はコンテナにボリュームマウントされており、ホスト側の変更が即時反映されます。
...
- **Excel直接描画**：`render_layout_to_xlsx` で openpyxl に直接書き込み
