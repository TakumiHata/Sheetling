...
PDFを読み込み、pdfplumber等を使用してテキスト・座標・フォント情報を抽出し、AIが生成したPythonソースコードを実行して高精度な2シートExcelファイルを生成するツールです。
...
- `src/`: アプリケーションのソースコード
  ...
  - `core/`: 解析・生成の主要ロジック
    ...
    - `executor.py`: AI出力Pythonコード実行 + 2シートExcel生成
...
当プロジェクトでは、以下の主要なPythonパッケージを使用しています。
- **`openpyxl`**: Excelファイル (.xlsx) の生成・編集、およびセルのスタイル・罫線の設定 (MIT License)
- **`pdfplumber`**: PDFファイルからのテキスト、座標、フォント、罫線情報の高精度な抽出 (MIT License)
...
- `*.md` — 人間可読な抽出結果
- `*_meta.json` — フォント・カラー情報（Phase 3で使用）
...
### Phase 3: Pythonコード実行 & 2シートExcel生成
