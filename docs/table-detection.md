...
`seen_edges` セット（`src/core/border_layout.py`）で (row, end_row, col, end_col, borders) のタプルをキーに重複を排除します。グリッドの量子化誤差で実質同一の矩形が二重登録されるのを防ぎます。
...
`_table_text_elements_from_2d()`（`src/core/table_layout.py`）は 2段階の戦略でテーブルセル内のテキストを配置します。
...
テーブル内テキスト配置は `src/core/table_layout.py`、テーブル外テキスト配置は `src/core/text_layout.py` に定義されています。
...
- [アーキテクチャ](architecture.md) — パイプライン全体のデータフロー・抽出データ構造
- [グリッドシステム](grid-system.md) — グリッド座標変換・table_border_rects の生成ロジック
- [チューニングガイド](tuning-guide.md) — テーブル検出の問題が起きた場合のトラブルシューティング
