...
`_compute_grid_coords()` 内で、テーブルの `table_cells` 2D配列からセルごとに border_rect を生成します。
...
`_is_near_duplicate()` でグリッド座標が ±1 以内の既登録要素を重複とみなします。グリッドの量子化誤差（±1行/列のずれ）で実質同一の矩形が二重登録されるのを防ぎます。
...
`_table_text_elements_from_2d()` は 2段階の戦略でテーブルセル内のテキストを配置します。
...
## 水平ギャップ分割 `_split_by_horizontal_gap()`
...
- テーブル外テキスト配置（`_auto_generate_layout()` 内）
