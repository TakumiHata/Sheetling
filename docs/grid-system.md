...
`_compute_grid_coords()` はまず全要素の座標からコンテンツ境界を検出します：
...
`_compute_grid_coords()` は変換後の座標を各要素に直接付与します：
...
|------|------|------|-------------------|-------------------|
| `1pt` | A4 | 高密度 | 61 × 42 | 89 × 30 |
| `2pt` | A4 | 中密度 | 37 × 42 | 58 × 30 |
| `1pt_a3` | A3 | 高密度 | 92 × 61 | 128 × 44 |
| `2pt_a3` | A3 | 中密度 | 57 × 61 | 79 × 44 |
...
`_setup_grid_params()` がページの寸法から判定します：
...
`_compute_grid_coords()` は計算したコンテンツ境界情報をページオブジェクトに保存します：
...
これらは後段の `_generate_border_preview()` で PDF 画像上の正しい位置に罫線を描画するために使用されます。
